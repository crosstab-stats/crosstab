...
- [x] **Export results / output — BUILT** (`core/output-export.js`, host-owned;
      **File ▸ Export output…**). Save the Output pane (tables, plots, notes) as a
      shareable report. The pane has **no result model** — it's append-only DOM in
      an *open* shadow root — so the renderer **clones the live `.results-root`
      markup and reuses the pane's own stylesheet** (faithful WYSIWYG), stripping
      interactive affordances (the redraw / save-plot buttons) and adding a title +
      "Generated by CrossTab · <date>" header and a print stylesheet. Both targets
      fall out of that one renderer:
