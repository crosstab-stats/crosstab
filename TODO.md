...
- [x] **#174 — DONE (2026-09-12). The gaps a real intro-soc methods class falls into
      ...
      **Two follow-ups this work surfaced, not yet done:**

      - [ ] **#174n — `recode … missing → X` folds only SQL NULLs, not designated missing
            CODES.** `recodeCaseSql`'s `from: 'missing'` rule emits `src IS NULL`, so a GSS
            variable whose 8/9 mean Don't know / No answer does not match it — the same gap
            the Count dialog had and now fixes (`missingSql`). Deliberately not changed
            here: `recodeVar` ops replay from the log, so altering the SQL changes the
            results of already-saved projects. Needs a decision about versioning the op or
            migrating, not a silent fix.
      - [ ] **#174o — the histogram's bin count is a dialog input, not a live control.**
            Setting it re-runs the analysis. The `categorical` chart model holds finished
            bins, so re-binding after render would need a real `histogram` chart kind that
            carries the raw values and bins at draw time — the shape #131 points at, and
            worth doing when another chart wants the same.
...
- [x] **#169 — DECLINED (owner, 2026-08-21): "let's not build anything we don't really
      ...
      2026-08-20). The counter-argument won: a stub that has to imitate `createTreeWalker`
      convincingly is a permanent maintenance cost, carried to catch a class of bug that
      hand-testing does catch, only later. Same test that refused vendoring — judge a
      safeguard by what maintaining it costs, not by the bug it would have caught.

      **The finding stands even though the fix was declined:** UI inside a sandboxed
      opaque-origin iframe is verified by a human or not at all, so any task touching it
      should budget for a browser pass rather than reading green tests as coverage.

- [x] **#170 — DONE (2026-08-21). NUL bytes in source, written as raw bytes instead of
      the `\0` escape.** Found while auditing #151. `grep` classes such a file as binary and
      prints "Binary file … matches" instead of the line, so a codebase-wide search silently
      skips it — which is how `core/rehome.js` nearly escaped that audit.

      **"Stray" was wrong, and the distinction is the whole job.** All three were INSIDE
      template literals, serving as key separators: `${pluginId}\0${name}` in
      codec-service.js:273, and `${doc}\0${code}` in builtin-agreement:183/188. Deleting
      them would have been a behaviour change, not a cleanup — with no separator,
      `doc="ab" code="c"` collides with `doc="a" code="bc"`, so two distinct codings would
      be counted as one and the agreement statistic would be quietly wrong. They were
      replaced with the two-character escape, which leaves the runtime string byte-identical
      ...
      **THREE OF THE FOUR KINDS HAVE SHIPPED** (verified 2026-08-21) — `violin`, `dots`
      ...
      - **SuperPlots — DEFERRED (owner, 2026-08-21).** Not merely a colour scheme; that
        ...
        **pseudoreplication**. The practice it targets: measure 100 cells in each of 3
        independent experiments, then plot all 300 as though they were 300 independent
        samples. n is inflated a hundredfold, the error bars shrink, and the p-value is
        spurious — the experiment was repeated 3 times, not 300. The plot makes the
        replicate structure visible: each biological replicate gets its own colour, its
        individual observations appear as small points in that colour, the **mean of each
        replicate** is drawn as a large marker in the same colour, and the summary and error
        bar are computed over the **replicate means (n = 3)**, not the raw points.

        **So the work is statistical, not decorative.** The drawing is cheap — `violin`,
        `dots` and the point overlay all exist. What is missing is a **replicate grouping
        variable** in the chart model and a summary layer that aggregates by it. Note what
        happens without one: `builtin-charts` computes error bars with `computeStats(raw)`
        over the raw observations in a cell, which is exactly the inflated-n statistic a
        SuperPlot was invented to expose. Correct on the charts it is offered on today,
        silently wrong on this kind, where the bar must come from the replicate means.

        Audience: the convention is close to universal in cell and molecular biology
        journals, so this is coverage for that readership rather than a styling choice.
